...
# 动阈值只有回归风险没有收益。
#   · V7.8.2 已回退，不要再试：曾把 Mac 候选上限 5 → 8，依据是"顶满 5 个被截断的
#     行命中率 14%、未截断的 38%"。实测候选数从 10 涨到 16 后召回一动不动（B=4/20、
#     C=6/20，连命中的行号都完全相同）。那个相关性是假的——候选多的行是背景杂乱的
#     行，而背景杂乱往往意味着拍得远/拍得糊，本来就难，不是被截断才失败的。
#     结论：候选上限不是瓶颈，加大只增成本。
